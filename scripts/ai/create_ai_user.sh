...
sudo kubeadm init phase certs apiserver --apiserver-cert-extra-sans $CODEDEPOT_CLUSTER_HOST
...
sudo openssl x509 -req -in $ADMIN_CSR \
...
kubectl apply -f ../scripts/ai/config/admin-user.yml

kubectl config set-cluster $CODEDEPOT_CLUSTER_NAME \
    --server=https://$CODEDEPOT_CLUSTER_HOST:6443 \
